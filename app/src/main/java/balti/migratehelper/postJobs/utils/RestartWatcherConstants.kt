package balti.migratehelper.postJobs.utils

import android.content.Context
import android.content.Intent
import android.os.Build

class RestartWatcherConstants {

    companion object {

        val WATCHER_PACKAGE_NAME = "balti.migratehelper.restartwatcher"
        val WATCHER_INTENT_DUMMY_ACTIVITY = "balti.migratehelper.restartwatcher.START"
        val WATCHER_INTENT_RECEIVER = "balti.migratehelper.restartwatcher.RECEIVER"

        val EXTRA_DO_START = "do_start"
        val EXTRA_ALIVE_CHECK = "alive_check"

        fun restartWatcher(context: Context, doStart: Boolean = false, intent: Intent? = null) {
            try {
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P)
                    context.startActivity(Intent(WATCHER_INTENT_DUMMY_ACTIVITY)
                        .putExtra(EXTRA_DO_START, doStart).apply {
                                intent?.let { putExtras(intent) }
                            }
                    )
            } catch (_: Exception){}
        }
    }

}